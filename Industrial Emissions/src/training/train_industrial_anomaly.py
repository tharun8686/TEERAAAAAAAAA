...
def train_anomaly_detector(X_train, features, models_dir):
    ...
    joblib.dump(iso_forest, os.path.join(models_dir, "industrial_anomaly_detector.pkl"))
    return iso_forest
...
def calibrate_and_evaluate(pipeline, X_train, y_train, X_val, y_val, models_dir, reports_dir):
    ...
    joblib.dump(calibrated_clf, os.path.join(models_dir, "industrial_calibrator.pkl"))
    ...
    if len(high_risk_indices) > 0:
        ...
        report = f"# Industrial Model Calibration Report\n\n"
        ...
        report += "The model probabilities are calibrated to match the actual rate of chemical leaks for better decision making.\n"
        
        with open(os.path.join(reports_dir, "industrial_calibration_report.md"), "w") as f:
            f.write(report)
...
def main():
    ...
    # Train Anomaly Detector on normal background air (class 0)
    ...
    anomaly_report = f"# Industrial Anomaly Detection Report\n\n"
    ...
    anomaly_report += f"- **Purpose:** Detects out-of-distribution pollutant levels or sudden leaks that deviate from typical ambient baseline air quality patterns.\n"
    
    with open(os.path.join(reports_dir, "industrial_anomaly_report.md"), "w") as f:
